...
public class AlphaIntegrationTest {
  ...
  private final TxEventServiceGrpc.TxEventServiceStub asyncStub = TxEventServiceGrpc.newStub(clientChannel);
  private final TxEventServiceGrpc.TxEventServiceBlockingStub blockingStub = TxEventServiceGrpc.newBlockingStub(clientChannel);
  ...
  public void compensateOnlyFailedGlobalTransaction() {
    ...
    TxEventServiceGrpc.TxEventServiceBlockingStub anotherBlockingStub = TxEventServiceGrpc.newBlockingStub(clientChannel);
  ...
  public void abortTimeoutSagaStartedEvent() {
    ...
    timeouts.forEach(timeout -> {
      assertThat(timeout.status(), Is.is(TaskStatus.DONE.name()));
  ...
  private boolean waitTillTimeoutDone() {
    for (TxTimeout txTimeout : timeoutEntityRepository.findAll()) {
      if (txTimeout.status().equals(TaskStatus.DONE.name())) {
        return true;
      }
    }
    return false;
